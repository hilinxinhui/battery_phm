...
<figure>
<figcaption>图2-4 NASA PCoE数据集B0005电池充电参数变化曲线</figcaption>
...
<figure>
<figcaption>图2-5 NASA PCoE数据集B0005电池放电参数变化曲线</figcaption>
...
<figure>
<figcaption>图2-7 NASA PCoE数据集B0005电池不同生命阶段充电过程参数对比示意图</figcaption>
...
美国国家航空航天局卓越故障预测研究中心（National Aeronautics and Space Administration Prognostics Center of Excellence，NASA PCoE）发布了一系列用于系统关键部件故障检测和健康管理的数据集，

本课题使用其中的5号数据集，这一电池数据集是最早发布也是最被广泛采用的电池数据集之一。
...
该数据集由丰田研究院（Toyota Research Institute，TRI）发布【文献】。该数据包含124颗单体磷酸铁锂（LFP）/石墨电芯循环数据（A123 Systems APR18650M1A）。

TRI数据集中的电池被分为三个批次（batch），本文使用第三个批次中前16颗电芯数据，具体电池信息、分组方式、变换方法和实验结果将在后文介绍。

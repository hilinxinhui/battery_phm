...
实验分组、实验使用的数据集划分方式和实验对应的模型间的区别已在本章第三节详细介绍，以下展示并分析实验结果。

如【图4-1】至【图4-4】为使用充放电过程中的电压、电流和温度数据，不进行时间序列-时间变换直接作为输入时模型在TRI数据集的四个分组上的实验结果。需要说明的是由于有重叠滑动窗口方法的引入，数据集中样本数量远大于循环数（上一章中从第二个时间窗口开始循环数与样本数一一对应），故本章图中横坐标表示样本编号。
